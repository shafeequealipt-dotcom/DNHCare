...
it. Written after the 2026-07-08 OpenRouter → Groq swap, which is the worked
example throughout.

---

## 1. LLM provider (currently Groq) — content generation
...
|---|---|
| `agent/bot/config.py` | `GROQ_API_KEY` / `GROQ_BASE_URL` env vars; `PRESET_MODELS` (fallback model list shown by `/models` if the live fetch fails); `DEFAULT_MODEL` |
...
| `agent/bot/bot.py` | `cmd_models()` calls `llm.list_models()`; user-facing text ("Fetching models from Groq…") |
...
2. **llm.py**: point the `OpenAI(base_url=..., api_key=...)` client at the new provider; update `list_models()`'s endpoint path and any provider-specific filtering (e.g. Groq serves non-chat models — Whisper speech-to-text, Orpheus TTS, prompt-guard classifiers — under the same `/models` endpoint, so `list_models()` excludes them by id substring).
3. **Test locally BEFORE touching Oracle** (see Testing pattern below) — a bad swap breaks every future post.
4. **Two easy-to-miss gotchas** (both bit us on the Groq swap):
   - **Cloudflare/WAF blocking Python's default `urllib` User-Agent.** `list_models()` uses a bare stdlib `urllib.request` GET (no SDK) — if that returns 403 while `curl` with the same key returns 200, add an explicit `User-Agent` header. The OpenAI SDK client used for `chat()` is unaffected — it sets its own UA.
...
5. **Docs**: update `.env.example`, `CAVEMAN.md`, `SETUP-ORACLE.md` — grep for the old provider name to catch every mention (`grep -rn "OldProviderName" agent/`).
6. **Secrets**: new key goes in `~/.zshrc` (source of truth) → Oracle's `agent/bot/.env` (via stdin, never echoed) → restart `dnhcare-bot`.
...
| File | What lives there |
|---|---|
| `agent/bot/config.py` | `GITHUB_TOKEN` (env `DNH_Github_Token`/`GITHUB_TOKEN`), `GITHUB_REPO`, `PUBLISH_BRANCH`, `REPO_DIR` |
